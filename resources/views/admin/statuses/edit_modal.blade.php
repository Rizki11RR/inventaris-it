<div class="modal fade" id="editStatusModal{{ $st->id }}" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('statuses.update', $st->id) }}" method="POST" class="modal-content border-0 rounded-4">
            @csrf @method('PUT')
            <div class="modal-header border-0 pt-4 px-4">
                <h5 class="fw-bold">Edit Status</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body px-4">
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted">NAMA STATUS</label>
                    <input type="text" name="nama_status" class="form-control bg-light border-0 py-2" value="{{ $st->nama_status }}" required>
                </div>
                <div class="mb-3">
                    ...
                    <select name="warna" class="form-select bg-light border-0 py-2" required>
                        ...
                        <option value="secondary" {{ $st->warna == 'secondary' ? 'selected' : '' }}>Abu-abu (Secondary)</option>
